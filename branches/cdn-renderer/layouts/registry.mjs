...
const { renderShellHeader, attachShellHeader, loadDashboards } = await load('./shell-header.mjs');
...
export async function renderRegistry({ frontmatter: fm, preamble, srcUrl }) {
  ...
  const header = renderShellHeader({
    ...
    meta: `${d.generated_at ? `<span class="reg-gen" title="This data is a SNAPSHOT, not live. Regenerate before trusting any number.">generated ${esc(String(d.generated_at).replace('T', ' ').replace(/\+.*$/, ''))}</span>` : ''}
